...
## Final V3 draft-shift data-integrity pass — 9 September 2026

Starting head: `1050b38fbee280a5e4be130c35a5765df3609b64`, same branch/base and Draft PR #57. Scope: `firestore.rules`, `scripts/test-scheduler-v3-emulator.mjs` and this report only.

`validDraftShiftV3()` now requires all 14 supported fields and rejects unknown keys. Dates use calendar-valid `v3Date`; V3-only `v3Time` permits `00:00` through `23:59`. IDs/references are bounded safe strings (1–100 characters), nullable template references are checked, employee names are non-empty strings bounded to 200 characters, override/cross-midnight fields are booleans, durations remain numeric in `(0,24]`, and source/schema/type remain constrained. Legacy `isTime`, `isIsoDate`, `validShift`, V2 code and authorization expressions were not modified.

Tests: new emulator assertions failed before the Rules fix and passed afterward. The full suite includes 182 negative create/update writes covering invalid dates/times, wrong field types, unsafe/empty/oversized strings, each missing required field and extra keys. Three positive read-back controls cover leap days in 2024, 2000 and 2400 with `00:00`/`23:59`, nullable/non-null template references, AUTO/MANUAL records and the 200-character name boundary. Existing repository roundtrip, version allocation, immutable history/PDF and authorization tests also pass.

Every requested regression command passed: `npm run test:scheduler-contract-v3` (304+17+8), `npm run test:scheduler-contract-v2` (2,118), `npm run qa:scheduler`, `npm run qa:scheduler-engine`, `npm run qa:repositories`, `npm run qa:public-readonly`, `npm run qa:tenant-authorization`, `npm run qa:auth-broker`, `npm run qa:export-security`, `npm run security:hardening`, `npm run security:integrity`, `npm run build`, plus the complete demo-only V3 emulator suite using the command above. Browser tests were not rerun for this Rules-only runtime delta. Existing build chunk-size and post-success emulator teardown warnings remain.

Security/rollback: only V3 structural validation was tightened; tenant/OWNER access is unchanged. No dependency, lockfile, GitHub Actions, credential, production data, deployment or migration change. No production readiness claim: Rules deployment still requires separate approval. Existing malformed V3 rows would need correction before a future update under these Rules; no production inventory or repair was attempted. Duration/time arithmetic and business warnings remain the existing application concerns. Rollback is to revert this scoped commit if needed while keeping V3 activation OFF; no publication/PDF deletion. Next action: human review of the same Draft PR, not merge/deploy.
